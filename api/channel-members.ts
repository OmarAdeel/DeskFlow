...
export default async function handler(request: Request, response: Response) {
  ...
  const additions = uniqueIds(body.additions);
  const deletions = uniqueIds(body.deletions);
  ...
  if (additions.length === 0 && deletions.length === 0) return sendJson(response, 200, { success: true });
  ...
  try {
    ...
    const callerMembership = await callerMembershipResponse.json() as { role?: string } | null;
    ...
    const callerChannelMembership = await callerChannelResponse.json() as { user_id?: string } | null;
    ...
    const requestedUserIds = [...new Set([...additions, ...deletions])];
    const organizationMembersResponse = await fetch(`${supabaseUrl}/rest/v1/organization_members?organization_id=eq.${encodeURIComponent(organizationId)}&user_id=in.(${requestedUserIds.map(encodeURIComponent).join(',')})&select=user_id`, {
      headers: adminHeaders
    });
    if (!organizationMembersResponse.ok) return sendJson(response, 400, { error: 'The selected users could not be validated.' });
    const organizationMembers = await organizationMembersResponse.json() as Array<{ user_id?: string }>;
    const validUserIds = new Set(organizationMembers.map(member => member.user_id).filter((id): id is string => Boolean(id)));
